...
st.markdown(
    f"""
    <div style="display:flex; flex-direction:column; gap:0.15rem; margin:0 0 1rem 0;">
        <div style="display:flex; align-items:baseline; gap:0.65rem; flex-wrap:wrap;">
            <h1 style="margin:0; line-height:1.05; font-size:2.6rem;">{APP_TITLE}</h1>
            <span style="font-size:1rem; color:#6b7280; line-height:1.05;">{APP_VERSION}</span>
        </div>
        <div style="font-size:1.02rem; color:#4b5563; line-height:1.35;">
            Das Programm berechnet die wichtigsten Zustände und Kennwerte eines einfachen Kältemittelkreislaufs.
        </div>
    </div>
    """,
...
if run:
    try:
        ...
        with st.expander("Anleitung"):
            st.markdown(
                """
                Dieses Tool dient zur **einfachen Kreislaufberechnung** von Kälteanlagen und Wärmepumpen und unterstützt dich bei der Bestimmung von **Kreislaufpunkten**, **Systemdaten**, **Stoffdaten** sowie der **Rohrleitungsdimensionierung**. Damit kannst du thermodynamische Zustände schnell überschlagen, Ergebnisse tabellarisch auswerten und bei Bedarf als CSV-Datei exportieren.
