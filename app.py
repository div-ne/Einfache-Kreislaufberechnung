...
def run_calculation(inputs):
    ...
    di_hg_dp, w_hg_dp, jg_hg_dp, dp_hg_dp = project_pipe(pre_hg, l_hg * 1.4, V_2, nu_2, rho_2, dp_max_hg)
    di_hg_dm, w_hg_dm, jg_hg_dm, dp_hg_dm = project_pipe(pre_hg, l_hg * 1.4, V_2, nu_2, rho_2, 1e5)

    di_fl_dp, w_fl_dp, jg_fl_dp, dp_fl_dp = project_pipe(pre_fl, l_fl * 1.4, V_3, nu_3, rho_3, dp_max_fl)
    di_fl_dm, w_fl_dm, jg_fl_dm, dp_fl_dm = project_pipe(pre_fl, l_fl * 1.4, V_3, nu_3, rho_3, 1e5)

    di_sl_dp, w_sl_dp, jg_sl_dp, dp_sl_dp = project_pipe(pre_sl, l_sl * 1.4, V_1, nu_1, rho_1, dp_max_sl)
    di_sl_dm, w_sl_dm, jg_sl_dm, dp_sl_dm = project_pipe(pre_sl, l_sl * 1.4, V_1, nu_1, rho_1, 1e5)
    ...
    df_pipes = pd.DataFrame([
        ["Heissgasleitung", "Auf Druckverlust optimiert", d_hg_dp, l_hg, round(w_hg_dp, 2), round(jg_hg_dp, 2), round(dp_hg_dp / 1e5, 2), A_M_hg_dp, V_i_hg_dp],
        ["Heissgasleitung", "Auf Durchmesser optimiert", d_hg_dm, l_hg, round(w_hg_dm, 2), round(jg_hg_dm, 2), round(dp_hg_dm / 1e5, 2), A_M_hg_dm, V_i_hg_dm],
        ["Flüssigkeitsleitung", "Auf Druckverlust optimiert", d_fl_dp, l_fl, round(w_fl_dp, 2), round(jg_fl_dp, 2), round(dp_fl_dp / 1e5, 2), A_M_fl_dp, V_i_fl_dp],
        ["Flüssigkeitsleitung", "Auf Durchmesser optimiert", d_fl_dm, l_fl, round(w_fl_dm, 2), round(jg_fl_dm, 2), round(dp_fl_dm / 1e5, 2), A_M_fl_dm, V_i_fl_dm],
        ["Saugleitung", "Auf Druckverlust optimiert", d_sl_dp, l_sl, round(w_sl_dp, 2), round(jg_sl_dp, 2), round(dp_sl_dp / 1e5, 2), A_M_sl_dp, V_i_sl_dp],
        ["Saugleitung", "Auf Durchmesser optimiert", d_sl_dm, l_sl, round(w_sl_dm, 2), round(jg_sl_dm, 2), round(dp_sl_dm / 1e5, 2), A_M_sl_dm, V_i_sl_dm],
    ], columns=[
...
title_col, ver_col = st.columns([8, 1])
with title_col:
    st.markdown(
        f'<h1 style="display:inline-block; margin:0;">{APP_TITLE}</h1> '
        f'<span style="font-size:1.1rem; color:#666; vertical-align:baseline;">{APP_VERSION}</span>',
...
with col1:
    ...
    lhg = st.number_input("Heissgasleitungslänge [m]", value=5.0) if show_pipe_inputs else 5
    lfl = st.number_input("Flüssigkeitsleitungslänge [m]", value=2.5) if show_pipe_inputs else 2.5
    lsl = st.number_input("Saugleitungslänge [m]", value=3) if show_pipe_inputs else 3
...
if run:
    try:
        ...
        with st.expander("Anleitung zu den Kreislaufpunkten"):
            st.markdown(
                """
                - **1:** Verdichtereingang und Saugleitungsausgang
                ...
                - **5:** Verdampferausgang und Saugleitungseingang
