...
with col1:
    ...
    with row4_col1:
        t0 = st.number_input("Verdampfungstemperatur [°C]", value=5)
    with row4_col2:
        dt0h = st.number_input("Verdampferüberhitzung [K]", value=6)

    dtsh = st.number_input("Saugleitungsüberhitzung [K]", value=9)
    ifpipes = st.selectbox("Rohrleitungsdimensionierung", ["Nein", "Ja"])

    show_pipe_inputs = ifpipes == "Ja"
    lhg = st.number_input("Heissgasleitungslänge [m]", value=5) if show_pipe_inputs else 5
